...
{
  static constexpr char urdf_path[] = "package://muul/models/block_pushing.urdf";
...
#if 0
  std::cout << "Setting inertial properties." << std::endl;
  for (BodyNode* const bodynode : skeleton->getBodyNodes())
  {
    if (bodynode->getNumCollisionShapes() > 1)
      throw std::runtime_error(str(
        ...
        % bodynode->getName() % skeleton->getName()
  ...
  std::cout << "Normalizing geometry." << std::endl;
  for (BodyNode* const bodynode : skeleton->getBodyNodes())
  {
